...
namespace GeometryAPI
{
    ...
    public class Cylinder : Figure, IAreaMeasurable, IVolumeMeasurable
    {
        ...
        {
            return Math.PI * this.Radius * this.Radius * (this.Center - this.Height).Magnitude;
        }

        public double GetArea()
        {
            return 2 * Math.PI * this.Radius * (this.Radius + (this.Center - this.Height).Magnitude);
        }
    ...
    //overriding FigureController
    public class NewFigureController : FigureController
    {
        public override void ExecuteFigureCreationCommand(string[] splitFigString)
        {
            switch (splitFigString[0])
            {
                ...
                case "cylinder":
                    {
                        Vector3D center = Vector3D.Parse(splitFigString[1]);
                        Vector3D height = Vector3D.Parse(splitFigString[2]);
                        double radius = double.Parse(splitFigString[3]);
                        currentFigure = new Cylinder(center, height, radius);
                        break;
                    }
            }
        ...
        protected override void ExecuteFigureInstanceCommand(string[] splitCommand)
        {
            switch (splitCommand[0])
            {
                case "area":
                    {
                        IAreaMeasurable currentFigure = this.currentFigure as IAreaMeasurable;
                        if (currentFigure != null)
                        {
                            Console.WriteLine("{0:0.00}", currentFigure.GetArea());
                        }
                        else
                        {
                ...
                case "normal":
                    {
                        IFlat currentFigure = this.currentFigure as IFlat;
                        if (currentFigure != null)
                        {
                            Console.WriteLine("{0:0.00}", currentFigure.GetNormal());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
                ...
                case "volume":
                    {
                        IVolumeMeasurable currentFigure = this.currentFigure as IVolumeMeasurable;
                        if (currentFigure != null)
                        {
                            Console.WriteLine("{0:0.00}", currentFigure.GetVolume());
                        }
                        else
                        {
                            Console.WriteLine("undefined");
...
namespace AcademyGeometry
{
    class Program
    {
        private static FigureController GetFigureController()
        {
            ...
            return new NewFigureController();
        }
