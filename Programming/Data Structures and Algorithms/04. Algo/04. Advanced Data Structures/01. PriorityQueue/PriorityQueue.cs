...
namespace PriorityQueue
{
    ...
    {
        private readonly List<T> heap; // List that stores the binary heap tree.
            ...
            : this((List<T>)null, comparer)
        {
            ...
            if (heap.Count > 1)
            {
                ...
                for (int index = (heap.Count - 1) >> 1; index >= 0; --index)
                {
                    DownHeap(index);
                }
            }
        ...
        public T[] ToArray()
        {
            T[] result = heap.ToArray();
            // We want to return the elements in the same order in which 
            // they are enumerated, which is sorted order, so we simply sort.
        ...
        public void CopyTo(T[] array, int arrayIndex)
        {
            ...
            // We want to return the elements in the same order in which they
            // are enumerated, which is sorted order, so we simply sort.
            Array.Sort(array, arrayIndex, heap.Count, Comparer);
        }
        ...
        private void UpHeap()
        {
            ...
            // Because we can't easily tell when parentIndex goes beyond 0, 
            // we check index instead; if that was already zero, then we're at the top
            while (index > 0 && Comparer.Compare(item, heap[parentIndex]) < 0)
            {
        ...
        private void DownHeap(int index)
        {
            ...
            bool compareResult = rightChild < count && 
                Comparer.Compare(heap[rightChild], heap[leftChild]) < 0;
            int smallestChild = (compareResult) ? rightChild : leftChild;

            while (smallestChild < count && Comparer.Compare(heap[smallestChild], item) < 0)
            {
                heap[index] = heap[smallestChild];
                index = smallestChild;
                leftChild = (index << 1) + 1;
                rightChild = leftChild + 1;
                smallestChild = (compareResult) ? rightChild : leftChild;
            }
            heap[index] = item;
        }
        ...
        public IEnumerator<T> GetEnumerator()
        {
            ...
            // the sorted copy.
            List<T> heapCopy = new List<T>(heap);
        ...
        {
            ((System.Collections.ICollection)heap).CopyTo(array, index);
        }
